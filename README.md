...
Currently we offer two database files that are inside the main directory MSSQLGenerationScript.sql contains the creations scripts for MSSQL server
...
### Installing
 ...
 open up powershell on windows or any terminal on your favorite linux distribution and execute the following command cd dotnet -c Release -r target patform for linux (linux-x64)
 -Once that is done you need to navigate to /bin/Release/netcoreapp3.1/publish take the content of that folder and deploy it on your favorite server like NGINX 
...
Please read [CONTRIBUTING.md](https://github.com/dwarf-industries/concoct-cloud/blob/master/Contributing.md) for details on our code of conduct, and the process for submitting pull requests to us.
...
This project is licensed under the Non-Profit Open Software License 3.0 (NPOSL-3.0) - see the [LICENSE.md](https://github.com/dwarf-industries/concoct-cloud/blob/master/License.md) file for details
